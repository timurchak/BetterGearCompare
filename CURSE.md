...
- Marks Best-in-Slot items in tooltips based on Wowhead guides.
- Includes a browsable BIS gear list with difficulty tier selector (`/bgc bis`).
...
BetterGearCompare includes a Best-in-Slot system powered by Wowhead guides for all 40 class/spec combinations.

- BIS items are marked in gear tooltips with a gold label.
- Type `/bgc bis` to open the BIS browser window.
- Select any spec and difficulty tier (Veteran / Champion / Hero / Myth).
- Items are displayed with correct item levels for the selected difficulty.
- Each item shows its source (boss name, dungeon, crafted, etc.) and a Wowhead link.
...
- BIS data is generated from Wowhead guides at release time.
